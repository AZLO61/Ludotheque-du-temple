 ...
 */
  ...
  // Harmonisé avec renderGameItem (même structure de carte, mêmes gabarits
  // de badge/difficulté/catégories) pour que l'onglet Vente ne soit pas visuellement
  // un site à part. Si le jeu vendu existe encore au catalogue (JEUX), on
  // réutilise ses catégories et sa difficulté ; sinon on affiche juste le
  // strict nécessaire (nom, prix, état).
  function renderItemAVendre(item){
    const cover = COVERS[item.nom];
    const vignette = cover
      ? '<div class="game-item__cover-wrap"><img class="game-item__cover" src="' + echapperHTML(cover) + '" alt="" loading="lazy" '
        + 'onerror="this.closest(\'.game-item__cover-wrap\').style.display=\'none\';"></div>'
      : '';
    const jeuCatalogue = JEUX.find(function(j){ return j.nom === item.nom; });
    ...
    html += '</div>';
    html += '<div class="game-item__badges"><span class="chip--new chip--sale">🛒 À vendre</span></div>';
    ...
    html += '</div>';
    if (jeuCatalogue) html += renderDifficulte(jeuCatalogue);
    html += '</div>';
    return html;
  }
  ...
  // Bascule vers l'onglet demandé : classes .active + aria-selected, tenus
  // synchronisés en un seul endroit (utilisé par le clic sur un onglet, le
  // clic sur le logo, et chercherJeu qui ramène vers "Recherche").
  function activerOnglet(tabName){
    document.querySelectorAll(".tab-btn").forEach(function(b){
      const estActif = b.getAttribute("data-tab") === tabName;
      b.classList.toggle("active", estActif);
      b.setAttribute("aria-selected", estActif ? "true" : "false");
    });
    document.querySelectorAll(".tab-content").forEach(function(c){
      c.classList.remove("active");
    });
    const contenu = document.getElementById(tabName + "-tab");
    if (contenu) contenu.classList.add("active");
  }

  function chercherJeu(nomJeu) {
    activerOnglet("search");

    // On ne mémorise la position que lors d'une VRAIE transition liste -> détail
    // (recherche vide avant ce clic) : si on est déjà sur un jeu précis
    ...
    // (pas de scrollTo ici : le jeu cliqué était déjà visible à l'écran,
    // le forcer à défiler serait redondant et gênant.)
  ...
  function renderGameItem(jeu, nomsAiment, categoriesEnChips) {
    ...
    // Fallback si la jaquette ne charge pas (lien externe hotlinké depuis
    // myludo.fr : peut casser sans prévenir) — on masque juste l'image plutôt
    // que de laisser l'icône "image cassée" du navigateur, moins parlante.
    const coverUrlEchappee = cover ? echapperHTML(cover).replace(/'/g, "\\'") : '';
    const vignette = cover
      ? '<div class="game-item__cover-wrap">'
      + '<img class="' + classeCouverture + '" src="' + echapperHTML(cover) + '" alt="" loading="lazy" '
      + 'tabindex="0" role="button" aria-label="Agrandir la jaquette de ' + echapperHTML(jeu.nom) + '" '
      + 'onclick="event.stopPropagation(); ouvrirLightboxImage(\'' + coverUrlEchappee + '\');" '
      + 'onkeydown="if(event.key===\'Enter\'||event.key===\' \'){event.preventDefault(); event.stopPropagation(); ouvrirLightboxImage(\'' + coverUrlEchappee + '\');}" '
      + 'onerror="this.closest(\'.game-item__cover-wrap\').style.display=\'none\';">'
      + '</div>'
      : '';
    ...
    // Ligne du nom : SEUL sur sa ligne, jamais concurrencé par les badges
    // (un jeu peut cumuler Nouveauté + Pépite + À vendre : les mettre sur
    // la même ligne qu'un nom long créait un empilement illisible sur
    // mobile). Les badges éventuels descendent sur leur propre ligne compacte.
    html += '<div class="game-item__row">';
    const nomEchappeJs = echapperHTML(jeu.nom).replace(/'/g, "\\'");
    html += '<span class="game-item__name" tabindex="0" role="button" '
          + 'onclick="event.stopPropagation(); chercherJeu(\'' + nomEchappeJs + '\')" '
          + 'onkeydown="if(event.key===\'Enter\'||event.key===\' \'){event.preventDefault(); event.stopPropagation(); chercherJeu(\'' + nomEchappeJs + '\');}">'
          + echapperHTML(jeu.nom) + '</span>';
    html += '</div>';
    if (badgeNew || badgePepite || badgeVente){
      html += '<div class="game-item__badges">' + badgeNew + badgePepite + badgeVente + '</div>';
    }
  ...
  // Construit le bloc "photo de la case + marqueurs" (cœurs toujours visibles,
  // points de recherche si une recherche textuelle précise est active).
  // Extrait de afficher() pour pouvoir le combiner dans un ordre différent
  // avec le bloc de jeux ci-dessous, selon qu'on est en mode hasard ou non.
  function construireBlocPhotoCase(c, jeuxDeLaCase, rechercheTexteActive){
    let bloc = '<div style="position: relative;">';
    bloc += '<button class="card__photo-wrap" data-photo="' + c.photo + '" aria-label="Agrandir la photo">';
    bloc += '<img class="card__photo" src="' + c.photo + '" alt="Case de rangement">';
    ...
    Object.keys(emplacementsCoeur).forEach(function(emplacement, index) {
      ...
      bloc += '<span class="spot-marker spot-marker--heart" style="left:' + left + '%; top:' + top + '%; --pulse-delay:' + decalage + ';" tabindex="0" role="button" aria-label="Coup de cœur à cet emplacement">'
    ...
    if (c.emplacements && rechercheTexteActive) {
      ...
      jeuxDeLaCase.forEach(function(jeu) {
        if (jeu.emplacement && c.emplacements[jeu.emplacement] && !emplacementsAffiches.has(jeu.emplacement) && !emplacementsCoeur[jeu.emplacement]) {
          ...
          const left = (pos.x / 800) * 100;
          const top = (pos.y / 800) * 100;
          bloc += '<span class="spot-marker" style="left:' + left + '%; top:' + top + '%;" title="Emplacement ' + jeu.emplacement + '"></span>';
        }
      });
    }
  ...
  // Construit le bloc "liste des jeux de cette case" (corps de carte).
  function construireBlocJeuxCase(jeuxDeLaCase){
    let bloc = '<div class="card__body">';
    jeuxDeLaCase.forEach(function(jeu) {
      bloc += renderGameItem(jeu, profilsAimant(jeu.nom), true);
    });
    bloc += '<p class="card__hint">Touche la photo pour l\'agrandir.</p></div>';
    return bloc;
  }
  ...
  // exactement au même endroit via "← Retour à la liste" (voir chercherJeu
  // et retourALaListe plus bas) — plutôt que de remonter tout en haut.
  let scrollAvantDetail = null;

  // État "mode hasard" : vrai uniquement juste après un clic sur "Je ne sais
  // pas quoi choisir", tant qu'on n'a pas changé de critère de recherche.
  // Une seule source de vérité pilote deux effets liés à ce mode :
  //  1. le bouton du haut se masque, remplacé par sa version "en ligne"
  //     ("Retenter ma chance"), qui prend exactement la place du texte
  //     d'astuce "Clique sur un jeu..." dans les résultats (voir afficher())
  //  2. le jeu tiré s'affiche AU-DESSUS de la photo de sa case (voir afficher())
  // Se désactive dès qu'on tape ou efface une recherche (texte ou croix ✕),
  // change de catégorie, modifie un filtre ou réinitialise — voir tous les
  // appels à definirModeHasard(false). Ne PAS en oublier un seul : c'est
  // précisément le genre d'oubli qui laisse le mode "actif" en arrière-plan
  // alors que l'utilisateur pense en être sorti (voir l'historique du ✕).
  let modeHasardActif = false;

  function definirModeHasard(actif){
    modeHasardActif = actif;
    // Le bouton du haut n'a plus lieu d'être une fois qu'on a lancé un tirage :
    // il "prend la place" du texte d'astuce dans les résultats à la place
    // (voir afficher()).
    const randomBtn = document.getElementById("randomGameBtn");
    if (randomBtn) randomBtn.hidden = actif;
  }
  ...
  function afficher() {
    ...
    if (modeHasardActif) {
      // Le bouton hasard "prend la place" du texte d'astuce : même
      // emplacement exact dans le flux (voir .random-btn--inline).
      html += '<button type="button" class="random-btn random-btn--inline" onclick="relancerHasard()">🎲 Retenter ma chance</button>';
    } else if (searchInput.value.trim().length > 0) {
      // Une recherche texte est active (tapée à la main, ou remplie
      // automatiquement par un clic sur un jeu via chercherJeu) : les tags/
      // filtres actifs restent, eux, intacts en mémoire (chercherJeu ne les
      // touche pas). Ce bouton rend ça visible et actionnable : il efface
      // UNIQUEMENT le texte de recherche, pour retrouver instantanément la
      // liste filtrée telle qu'elle était avant le clic — sans avoir à
      // rejouer les filtres à la main.
      html += '<button type="button" class="back-to-list-btn" onclick="retourALaListe()">← Retour à la liste</button>';
    } else {
    ...
    caseIds.forEach(function(caseId) {
      ...
      const blocPhoto = construireBlocPhotoCase(c, jeuxDeLaCase, rechercheTexteActive);
      const blocJeux = construireBlocJeuxCase(jeuxDeLaCase);

      // En mode "Je ne sais pas quoi choisir", le jeu tiré s'affiche AU-DESSUS
      // de la photo de sa case : le résultat doit être visible immédiatement,
      // sans avoir à défiler jusqu'à la photo. Dès qu'on quitte ce mode
      // (recherche texte manuelle, changement de catégorie, filtres,
      // réinitialisation — voir definirModeHasard), l'ordre classique
      // reprend : photo d'abord, détail du/des jeu(x) ensuite.
      html += '<article class="card">';
      html += modeHasardActif ? (blocJeux + blocPhoto) : (blocPhoto + blocJeux);
      html += '</article>';
    });
  ...
  // Tire un jeu au hasard parmi les résultats actuellement filtrés
  // (tags/joueurs/durée) et le révèle avec le même mécanisme que le clic sur
  // un jeu depuis les recommandations (chercherJeu), avec une petite mise en
  // avant visuelle sur sa case. Fonction globale (déclarée au premier niveau,
  // donc accessible via window) : utilisée à la fois par le bouton du haut
  // (#randomGameBtn) et par sa version "en ligne" générée dans afficher(),
  // appelée depuis un attribut onclick.
  // Efface UNIQUEMENT le texte de recherche (tags/filtres actifs préservés),
  // pour revenir à la liste filtrée telle qu'elle était avant de cliquer sur
  // un jeu précis. Fonction globale : appelée depuis le bouton "← Retour à
  // la liste" généré dans afficher() via un attribut onclick.
  function retourALaListe(){
    searchInput.value = "";
    clearBtn.hidden = true;
    afficher();
    // On restaure la position EXACTE où on était dans la liste avant le
    // détail, plutôt que de laisser le navigateur (ou un scroll par défaut)
    // remonter tout en haut.
    if (scrollAvantDetail !== null) {
      window.scrollTo({ top: scrollAvantDetail, behavior: "auto" });
      scrollAvantDetail = null;
    }
  }

  function relancerHasard(){
  ...
  if (randomGameBtn) {
    randomGameBtn.addEventListener("click", relancerHasard);
  }

  // Bouton "remonter en haut" : élément fixe permanent (comme le bouton
  // jour/nuit), visible dès le chargement de la page, sur tous les onglets.
